...
int main(int argc,char *argv[])
{
	...
	static long int seed,seed0;	
	 ...
	 seed0 = -atol(argv[2]);
      ...
      FILE *archivo;
      FILE *guardar;
      guardar = fopen("casos_oran.txt","w");
  ...
  //       if (i == DAYS - 1) { printf("%d\t%d\t%.0f\n",corr,i,importados[i]);
   ...
   //printf ("MOSQUITO: %d\t%d\n",LIMPIA,inter_mosco);
      ...
      //Calculo los vectores suceptibles que se van VD[][] 
      for ( j = 0; j < RAD_CENSAL ; j++ ){
	  ...
	  if ( month < i ){
		  ...
		  save_inc_ya[year_epid]   =   save_inc_ya[year_epid]  +  inc_y;
		  
		  save_inc_ypc[year_epid]  =   save_inc_ypc[year_epid] +  inc_yp;
		  
		  inter_LP[year_epid]      =   inter_LP[year_epid]     +  inter_larv_pup;
          
		  inter_M[year_epid]       =   inter_M[year_epid]      +  inter_mosco;
  ...
  //printf("\n");
  printf("Oran_week_model\n");
  

  printf("#year\t<inc>\t<imp>\t<larv>\t<mosq>\n");
  aux2 = (float)ITERAR;
  ...
  while ( i < YEARS ){
	  ...
	  for( j=0; j < ITERAR - 1 ; j++ ){
		  inc_y = inc_y + (float)(save_inc_ya[i]);
		  inc_yp = inc_yp + (float)(save_inc_ypc[i]);
		  MV = MV + inter_LP[i];
		  MH = MH + inter_M[i];
	 ...
	 // }
	  ...
	  printf("%d\t%.2f\t %.2f\t %.2f\t%.2f\n",BEGIN_YEAR+i,inc_y,inc_yp,MV,MH);
      i++;
  }
